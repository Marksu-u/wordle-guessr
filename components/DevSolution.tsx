...
export default function DevSolution({ solution }: { solution: string }) {
  ...
  return (
    <div className="fixed right-4 bottom-4 z-50 flex flex-col items-end gap-2">
      {visible && (
        <span className="rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 font-mono text-sm font-black tracking-widest text-green-400 uppercase shadow-xl">
          {solution}
        </span>
      )}
      ...
      <button
        ...
        className="flex h-8 w-8 items-center justify-center rounded-full border border-zinc-700 bg-zinc-900 text-zinc-500 opacity-30 transition-opacity hover:opacity-100"
      >
        <svg
